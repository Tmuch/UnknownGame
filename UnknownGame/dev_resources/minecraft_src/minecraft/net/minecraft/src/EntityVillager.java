...
public class EntityVillager extends EntityAgeable implements IMerchant, INpc
{
    ...
    {
        List list = ModLoader.getTrades(this.getProfession());
    ...
    static
    {
        ...
        while (i$.hasNext())
        {
            ...
            if (entry.buying)
            {
                ...
                {
                    villagerStockList.put(Integer.valueOf(entry.id), new Tuple(Integer.valueOf(entry.min), Integer.valueOf(entry.max)));
                }
            }
            else if (entry.min > 0 && entry.max > 0)
            {
                blacksmithSellingList.put(Integer.valueOf(entry.id), new Tuple(Integer.valueOf(entry.min), Integer.valueOf(entry.max)));
            }
        }
    }
}
